...
一个可部署至 GitHub Pages 的零构建静态站点：算法执行逻辑在题目包生成轨迹，浏览器只播放帧数据。Phase 1 提供通用播放器、数组指针与 DP 网格组件，以及 Two Sum II 的完整 demo。
...
1. 复制 `problems/_template/`，并把目录名与三个文件中的占位 ID 改为实际题目 ID。
