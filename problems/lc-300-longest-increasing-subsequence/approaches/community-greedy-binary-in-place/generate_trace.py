...
def generate() -> dict:
    ...
    frames = [{"step": 0, "description": "把 nums 的前缀视为 g", "array": nums.copy(), "pointers": {"ng": 0}, "highlightIndices": [], "note": "ng = 0；尚无候选。之后 nums[0:ng] 复用为 g，不额外分配数组。"}]
    for read, x in enumerate(INPUT):
        slot = bisect.bisect_left(nums, x, 0, ng)
        nums[slot] = x
        extended = slot == ng
        if extended: ng += 1
        frames.append({"step": len(frames), "description": f"处理原输入 nums[{read}] = {x}", "array": nums.copy(), "pointers": {"slot": slot, "ng": ng - 1}, "highlightIndices": [slot], "window": {"start": 0, "end": ng - 1}, "note": (f"在有效前缀 [0, {ng - 1}] 中二分定位 {slot}；写回 nums[{slot}] = {x}。" + (" 位置原在末尾，ng 加一。" if extended else " 替换同长度候选的结尾，ng 不变。"))})
