...
export async function mountListPage() {
  ...
  try {
    ...
    const render = () => { const query = search.value.trim().toLocaleLowerCase(); const filtered = problems.filter((problem) => (!query || `${problem.title} ${problem.tags.join(' ')}`.toLocaleLowerCase().includes(query)) && (!difficulty.value || problem.difficulty === difficulty.value) && (!tag.value || problem.tags.includes(tag.value))); results.innerHTML = filtered.length ? filtered.map((problem) => `<a class="problem-card panel" href="problems/${encodeURIComponent(problem.id)}/index.html"><p class="eyebrow">${escapeHtml(problem.vizType)}</p><h2>${escapeHtml(problem.title)}</h2><p class="difficulty-${escapeHtml(problem.difficulty)}">${escapeHtml(problem.difficulty)}</p><div class="card-meta">${problem.tags.map((item) => `<span class="tag">${escapeHtml(item)}</span>`).join('')}</div></a>`).join('') : '<div class="empty-state panel">没有符合条件的题目。</div>'; };
    [search, difficulty, tag].forEach((element) => element.addEventListener('input', render)); render();
