...
test('problem pages load a local Prism runtime and highlight inserted code lines', async () => {
  ...
  assert.match(assembly, /window\.Prism\.highlight\(lineText, grammar, languageKey\)/);
  assert.match(assembly, /line\.className = 'code-line'/);
  assert.doesNotMatch(assembly, /highlightAllUnder/);
